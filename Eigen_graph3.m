...
fileID = fopen([Dirsave_data,'4Eigen_',num2str(1),'_Stokesmp.txt'],'w');
Gammavar = [10.^(-mp(linspace(0,12,101)))];
    ...
    for k=1:length(Apoint)
        ...
        for j=1:length(Astencial)
            for sj=1:length(Gammavar)
                ...
                Settings.Params = [mp('5e-06'),mp('0.0'),mp('0.0'),mp('0.5'),Gammavar(sj),Avar_muu(lk)];
