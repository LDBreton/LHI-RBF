...
addpath('locallibs/');
mkdir('../Data_dirich/')
Dirsave_data = '../Data/LHI_Stokes_pre_hibridKenerlR3/';
mp.Digits(100);
mkdir(Dirsave_data);
...
for lk=1:length(Avar_muu)
    for k=1:length(Apoint)
        ...
        [P_sc2,P_fc2] = Mesh_gen(1/21,1/21,1.0);
        for j=1:length(Astencial)
            ...
            Indices_sup2 = LHI_index2(P_sc,P_sc2, P_fc2,Indices_sup,Astencial(j));

            
            write_files(P_sc,P_fc,Indices_sup,distancepp,dir_name);            
            Settings = write_files2(P_sc2,P_fc2,Indices_sup,dir_name);
            
            Settings.programa = 'Cplusplus/LHI_Wegths_Save_newcenters.out';
            Settings.lib = 'Cplusplus/hibridKenerlR3.so';
            ...
            ParamCdiv = 1.0;
            %{'gammass','c','cpre','mus'};
            Settings.Params = [mp('10000.0'),mp('1.0'),mp('1.0'),Avar_muu(lk)];

            disp('calculando pesos')
            tic;
            Compute_matrixs_general_save2(Settings,[5,6,7,8],length(P_sc));
            timecomputed=toc;
            timecomputed
