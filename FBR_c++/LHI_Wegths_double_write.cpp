...
vector< Real_scalar > LocalRcond(Npesos_calcular);
cout << Npuntos_sc << " " << Npesos_calcular << endl;

cout << "Beging of weitgh calculation" << endl;
...
cout << "End of weitgh calculation" << endl;
cout << "Writing files" << endl;
//////////////////////Pinting the weigths/////////////
ofstream pesos_file;
std::streambuf *coutbuf = std::cout.rdbuf();
std::cout.rdbuf(pesos_file.rdbuf());
std::string filename_pesos = string(argv[13]) + "pesos_file.txt" ;
pesos_file.open (filename_pesos);
{
...
ofstream rcond_file;
std::cout.rdbuf(rcond_file.rdbuf());
std::string filename_rcond = string(argv[13]) + "rcond_file.txt" ;
rcond_file.open (filename_rcond);

	for (int k = 0; k < Npesos_calcular; ++k) {
		cout << LocalRcond[k] << '\n';
}
rcond_file.close();
