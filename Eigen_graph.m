...
addpath('locallibs/');
mkdir('../Data_dirich/')
Dirsave_data = '../Data/3hibridKenerlR73LimGood/';
mp.Digits(100);
mkdir(Dirsave_data);
...
fileID = fopen([Dirsave_data,'nociEigen_',num2str(1),'_Stokesmp.txt'],'w');
%Gammavar = [0,10.^(-mp(linspace(0,12,101)))];
Gammavar = linspace(0.0,0.01,101);
    ...
    for k=1:length(Apoint)
        ...
        for j=1:length(Astencial)
            for sj=1:length(Gammavar)
                ...
                Settings.Params = [Gammavar(sj),mp('1e-07'),mp('0.0'),mp('0.6'),mp('5e-05'),9,5,Avar_muu(lk)];
                ...
                [pesos,Pre] = read_weights_mp2(dir_name,[5 6 7 8]);
                ...
                ML = full(speye(size(SY))-double(SL)) ;
                WY = full(double(SY)) ;
                e = eig(-WY,ML);
                Idx = (real(e) > 0);
