function [L2EvectY,L2EGradP,LinftyEvectY,LinftyEGradP] = compute_error_dirich_new(Pre,var_muu)
...
RHS = [F1;F2;zeros(size(F1))]-Pre.Mbf*[F1;F2;B1;B2] ;
Y_aprox = Pre.Myp\RHS;
RHSP = [Y_aprox;B1;B2;F1;F2];
PGrad_aprox = Pre.MgradP*RHSP;

TRI = delaunayTriangulation(double(P_sc(:,1)),double(P_sc(:,2)));
Nin = length(P_sc(:,2));
ErrorY =  Y_aprox(1:2*Nin)-[Y_sol1;Y_sol2];
