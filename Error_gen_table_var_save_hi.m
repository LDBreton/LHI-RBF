...
addpath('locallibs/');
mkdir('../Data_dirich/')
Dirsave_data = '../Data/LHI_Stokes_hibi/';
mp.Digits(100);
mkdir(Dirsave_data);
...
for lk=1:length(Avar_muu)
    for k=1:length(Apoint)
        ...
        for j=1:length(Astencial)
            ...
            Settings.lib = 'Cplusplus/LHI_Stokes_pre_hibridKenerlR3gimq.so';
            ...
            ParamCdiv = 0.0952;
            %{'gammass1','gammass2','c','cpre','consts','mus'};
            Settings.Params = [1.0,0.0,0.1,0.00001,0.000000001,Avar_muu(lk)];
